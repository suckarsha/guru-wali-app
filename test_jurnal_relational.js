...
async function testRelationalInsert() {
  ...
  const payload = {
    ...
    topik: 'Test Relational',
    tindak_lanjut: 'Test Relational',
    guru_id: null
  };

  const { data, error } = await supabase.from('guidance_journals').insert([payload]).select(`*, students(name, classes(name)), auth_users(email)`).single();
  
  console.log('Result Data:', data);
  console.log('Result Error:', error);
}

testRelationalInsert();
