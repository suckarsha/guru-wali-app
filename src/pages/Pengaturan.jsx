...
export default function Pengaturan() {
  ...
  const handleClearData = async () => {
    if (confirm('PERINGATAN SEVERA!\nSemua data di dalam DATABASE SUPABASE (Siswa, Kelas, Kehadiran, Jurnal, dan Pengumuman) serta data lokal Anda akan dihapus permanen.\n\nApakah Anda benar-benar yakin ingin menghancurkan semua data?')) {
      try {
        setIsSaving(true);
        await settingService.clearAllDatabase();
        localStorage.clear();
        alert('Database Supabase dan data lokal berhasil dikosongkan secara permanen. Aplikasi akan dimuat ulang.');
        logout();
        navigate('/login');
        window.location.reload();
      } catch (error) {
        console.error(error);
        alert('Gagal mengosongkan database. Pastikan Anda memiliki koneksi internet dan izin admin.');
      } finally {
        setIsSaving(false);
      }
    }
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-red-50/50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-xl gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Hapus semua data dari database (Murid, Kelas, Kehadiran, Jurnal, Pengumuman) serta penyimpanan lokal. Sistem akan logout setelah selesai.</p>
                </div>
                <button 
                  disabled={isSaving}
                  onClick={handleClearData} 
                  className="flex items-center justify-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-semibold transition-colors whitespace-nowrap shadow-sm shadow-red-200 dark:shadow-red-900/30 disabled:opacity-50"
                >
                  <Trash2 size={16} />
                  Hapus Data
                </button>
