...
async function testFinalRelations() {
  ...
  const payload = {
    ...
    topik: 'Test Direct Call Final',
    tindak_lanjut: 'Direct test',
    guru_id: null
  };

  console.log('Inserting with relation payload...');
  const { data, error } = await supabase.from('guidance_journals').insert([payload]).select(`*, students(name, classes(name))`).single();
  
  if (error) {
    console.log('Select Error details:', error);
  } else {
